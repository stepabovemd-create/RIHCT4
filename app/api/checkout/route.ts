...
// NEXT.JS (App Router) + Stripe webhook handler + Postmark email (with same-domain fallback)
...
export async function POST(req: NextRequest) {
  ...
  console.log('   env STRIPE_WEBHOOK_SECRET?', Boolean(process.env.STRIPE_WEBHOOK_SECRET));
  console.log('   env STRIPE_SECRET_KEY?', Boolean(process.env.STRIPE_SECRET_KEY));
  ...
  try {
    ...
    if (event.type === 'invoice.payment_succeeded') {
      ...
      // ---- Send email via Postmark (with fallback recipient + loud logs) ----
      const to = email || (process.env.TEST_RECIPIENT as string | undefined);

      if (process.env.POSTMARK_TOKEN && process.env.SENDER_EMAIL && to) {
        ...
        if (res.status === 422 && body.includes('"ErrorCode":412')) {
          console.log(
            '📧 Postmark blocked sending to external recipients (account pending approval). Use same-domain recipient or finish domain approval.'
          );
        } else {
        ...
        console.log(
          '📧 Skipped email (missing POSTMARK_TOKEN/SENDER_EMAIL or no recipient; got email=%s, TEST_RECIPIENT=%s)',
          email,
